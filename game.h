...
struct Game {
    ...
    SDL_Texture *bulletTexture, *enemyTexture, *enemyBulletTexture, *background, *explosionTexture,*heartTexture, *bossTexture, *khung, *khung2, *roll, *princessTexture, *guide, *guide1, *huongdan,
                                *bulletbossTexture, *playTexture, *muteTexture, *unmuteTexture, *playbutton1Texture, *playbutton2Texture, *quit1buttonTexture, *skill00, *skill01, *skill10, *skill11,
                                *quit2buttonTexture, *shieldTexture, *shield2Texture, *shield3Texture, * thunderTexture, *lightTexture, *thunderstormTexture, *winTexture, *loseTexture;
    ...
    void init(Graphics& graphics)
    {
        ...
        thunderTexture = graphics.loadTexture("img/thunder.png");
        lightTexture = graphics.loadTexture("img/light.png");
        thunderstormTexture = graphics.loadTexture("img/thunderstorm.png");
        princessTexture = graphics.loadTexture("img/princess.png");
        skill00 = graphics.loadTexture("img/00.png");
        skill01 = graphics.loadTexture("img/01.png");
        skill10 = graphics.loadTexture("img/10.png");
        skill11 = graphics.loadTexture("img/11.png");
        guide1 =  graphics.loadTexture("img/guide1.png");
        guide = graphics.loadTexture("img/guide.png");
        huongdan = graphics.loadTexture("img/huongdan.png");
        winTexture = graphics.loadTexture("img/win.png");
        loseTexture = graphics.loadTexture("img/lose.png");
        reset();
    }
    ...
    void drawguidebutton(Graphics &graphics)
    {
        graphics.renderTexture(guide, 970, 350);
    }

    void drawguide1button(Graphics &graphics)
    {
        graphics.renderTexture(guide1, 920, 310);
    }

    void drawhuongdan(Graphics &graphics)
    {
        graphics.renderTexture(huongdan, 95, 78);
    }
    ...
    void doPlayer(int keyboard[])
    {
        ...
        if (keyboard[SDL_SCANCODE_E] and created){
            createshield(shield);
            created = 0;
        }
        if (keyboard[SDL_SCANCODE_R] and thundered){
            thunder = true;
            thundered = false;
            clearEnemies();
            boss->health -= 2;
    ...
    {
        graphics.renderTexture(winTexture, 0, 0);
    }

    void endgamelose(Graphics &graphics)
    {
        graphics.renderTexture(loseTexture, 0, 0);
    }
    ...
    {
        if(thundered) graphics.renderTexture(skill11, 0, 200);
        else graphics.renderTexture(skill10, 0, 200);
        if(!created) graphics.renderTexture(skill00, 0, 280);
        else graphics.renderTexture(skill01, 0, 280);
    }
    ...
    void draw(Graphics& graphics)
    {
        ...
        graphics.renderTexture(princessTexture, 850, 80);
        graphics.renderTexture(khung2, 850, 0);
        graphics.renderTexture(roll, 330, 0);
        drawskill(graphics);

        for(int heart = 0; heart <= playerdiecount ; heart++){
        ...
        if (thunder){
            graphics.renderTexture(thunderTexture, 0, 0);
            SDL_Delay(500);
            graphics.renderTexture(lightTexture, 0, 0);
            thunder = false;
        }
